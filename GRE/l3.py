from math import comb, floor, exp, factorial, sqrt, pi, ceil
...
mu = n * p  # μ = 9 * 0.2 = 1.8
sigma = sqrt(n * p * q)  # σ = sqrt(9 * 0.2 * 0.8) ≈ sqrt(1.44) ≈ 1.2

# Функция для вычисления вероятности P(X = k) по формуле Лапласа
def laplace_probability(k, mu, sigma):
    return (1 / (sqrt(2 * pi) * sigma)) * exp(-((k - mu)**2) / (2 * sigma**2))

# Функция для вычисления интервальной вероятности P(a <= X <= b)
def laplace_integral(a, b, mu, sigma):
...
k1 = 4
prob_1 = laplace_probability(k1, mu, sigma)

# 2a) Вероятность того, что будет продано менее 2 пакетов (X < 2)
prob_2a = laplace_integral(0, 1, mu, sigma)  # P(X < 2) = P(0 <= X <= 1)

# 2b) Вероятность того, что будет продано не более 2 пакетов (X <= 2)
prob_2b = laplace_integral(0, 2, mu, sigma)  # P(X <= 2) = P(0 <= X <= 2)

# 2c) Вероятность того, что будет продано хотя бы 2 пакета (X >= 2)
prob_2c = 1 - laplace_integral(0, 1, mu, sigma)  # P(X >= 2) = 1 - P(X < 2)

# 2d) Наивероятнейшее число проданных пакетов
# Наивероятнейшее число k определяется как целое число, ближайшее к (n + 1) * p
most_probable_k = round((n + 1) * p)

# Вычисляем вероятность для наивероятнейшего числа
prob_most_probable_k = laplace_probability(most_probable_k, mu, sigma)
...
print(f"2d) Наивероятнейшее число проданных пакетов: {most_probable_k}, вероятность: {prob_most_probable_k:.4f}")
...
# Параметры задачи
p = 1 / 6  # вероятность выпадения тройки
q = 1 - p  # вероятность не выпадения тройки
k_0 = 10  # наивероятнейшее число успехов

# Математическое ожидание и стандартное отклонение
# Для нормального приближения используем формулы μ = n * p и σ = sqrt(n * p * q)

# Находим минимальное n, чтобы выполнялось условие μ - 1 <= k_0 <= μ
# μ = n * p => n * p - 1 <= k_0 <= n * p
# Перепишем как два неравенства:
# 1) n * p >= k_0
# 2) n * p - 1 <= k_0

# Решаем первое неравенство: n * p >= k_0 => n >= k_0 / p
n_min = ceil(k_0 / p)

# Проверяем второе неравенство: n * p - 1 <= k_0 => n <= (k_0 + 1) / p
n_max = int((k_0 + 1) / p)

# Минимальное значение n, удовлетворяющее обоим неравенствам
n = max(n_min, n_max)
...
# Левая и правая границы для проверки
lower_bound = mu - 1  # левая граница
upper_bound = mu       # правая граница

# Вывод результатов
print(f"Минимальное количество подбрасываний n: {n}")
print(f"Проверка: [{lower_bound:.1f}, {upper_bound:.1f}]")
...
# Математическое ожидание и стандартное отклонение
mu = n * p  # μ = 243 * 0.25 = 60.75
sigma = sqrt(n * p * q)  # σ = sqrt(243 * 0.25 * 0.75) ≈ sqrt(45.5625) ≈ 6.75

# Формула Лапласа для вычисления вероятности P(X = k)
def laplace_probability(k, mu, sigma):
    return (1 / (sqrt(2 * pi) * sigma)) * exp(-((k - mu)**2) / (2 * sigma**2))

# Вычисляем вероятность того, что событие A наступит ровно 70 раз
probability = laplace_probability(k, mu, sigma)

# Вывод результата
print(f"Вероятность того, что событие A наступит ровно {k} раз: {probability:.6f}")
...
lambda_poisson = n * p  # λ = 1825 * (1/365) = 5

# Формула распределения Пуассона: P(X = k) = (λ^k * e^(-λ)) / k!
def poisson_probability(k, lambda_p):
    return (lambda_p**k * exp(-lambda_p)) / factorial(k)

# Вычисляем вероятность того, что ровно у четырех студентов день рождения 1 сентября
k = 4
probability_exact_4 = poisson_probability(k, lambda_poisson)
...
q = 1 - p  # вероятность не повреждения

# Математическое ожидание и стандартное отклонение
mu = n * p  # μ = 10000 * 0.0002 = 2
sigma = sqrt(n * p * q)  # σ = sqrt(10000 * 0.0002 * 0.9998) ≈ sqrt(1.9996) ≈ 1.414

# Функция Лапласа для вычисления вероятности P(X = k)
def laplace_probability(k, mu, sigma):
    return (1 / (sqrt(2 * pi) * sigma)) * exp(-((k - mu)**2) / (2 * sigma**2))
...
# Задача 1: Вероятность того, что будет повреждено ровно три изделия (P(X = 3))
k1 = 3
probability_exact_3 = laplace_probability(k1, mu, sigma)

# Задача 2: Вероятность того, что будет повреждено по крайней мере три изделия (P(X >= 3))
# P(X >= 3) = 1 - P(X < 3) = 1 - P(0 <= X <= 2)
probability_at_least_3 = 1 - laplace_integral(0, 2, mu, sigma)

# Задача 3: Вероятность того, что не будет повреждено ровно 9997 изделий
# Это эквивалентно вероятности того, что будет повреждено ровно 3 изделия (P(X = 3))
probability_not_damaged_9997 = probability_exact_3

# Задача 4: Вероятность того, что не будет повреждено хотя бы 9997 изделий
# Это эквивалентно вероятности того, что будет повреждено не более двух изделий (P(X <= 2))
probability_not_damaged_at_least_9997 = laplace_integral(0, 2, mu, sigma)
