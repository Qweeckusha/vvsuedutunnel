...
# Вероятность наличия заболевания среди пациентов (априорная вероятность)
# Используем формулу: P(заболевание) = P(положительный тест) * P(выявление | заболевание) /
#                     [P(положительный тест) * P(выявление | заболевание) + P(положительный тест | здоров) * P(здоров)]
# Подставляем известные значения:
# Пусть x = P(заболевание), тогда P(здоров) = 1 - x

# Находим вероятность положительного теста при отсутствии заболевания
P_positive_given_no_disease = 1 - P_negative_given_no_disease

# Формула Байеса для нахождения вероятности наличия заболевания при положительном тесте:
# P(заболевание | положительный тест) = P(положительный тест | заболевание) * P(заболевание) / P(положительный тест)

# Предполагаем априорную вероятность заболевания как неизвестную x и решаем уравнение:
# P(заболевание) = P(положительный тест | заболевание) * P(заболевание) /
#                   [P(положительный тест | заболевание) * P(заболевание) + P(положительный тест | здоров) * P(здоров)]

# Подставляем числа:
# P(заболевание | положительный тест) = (0.86 * 0.10) / [(0.86 * 0.10) + (0.06 * 0.90)]

P_disease_given_positive = (P_positive_given_disease * P_positive) / \
                           ((P_positive_given_disease * P_positive) + (P_positive_given_no_disease * (1 - P_positive)))
